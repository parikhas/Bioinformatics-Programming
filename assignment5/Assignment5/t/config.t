...
BEGIN { use_ok('Assignment5::Config', qw(getErrorString4WrongNumberArguments getUnigeneDirectory
  getUnigeneExtension
  getHostKeywords)) }
#dies when give it a bogus file name
dies_ok { getErrorString4WrongNumberArguments(1) } 'dies ok when an argument is passed';
dies_ok { getUnigeneDirectory(1) } 'dies ok when an argument is passed';
dies_ok { getUnigeneExtension(1) } 'dies ok when an argument is passed';
dies_ok { getHostKeywords(1) } 'dies ok when an argument is passed';