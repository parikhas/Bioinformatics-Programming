...
# check to see if we got a file to parse
unless ($IFILE) {
	die
...
while (<$fhIn>) {
	...
	$sym = uc $sym;
	my $desc = $columns[1];
	# put symbol as the key and description as the value in the hash
	$sym_desc{$sym} = $desc;
}
...
while ( my $input = <STDIN> ) {
	...
	$input = uc $input;
	# If user input matches the gene symbol, print the gene description
	if ( exists $sym_desc{$input} ) {
		say $input, " found! Here is the description:";
		say $sym_desc{$input};
		say "Enter gene name of interest. Type quit to exit:";
	}
	# if user inout is quit then, end the program
	elsif ( $input eq 'QUIT' ) {
		last;
	}
	else {
